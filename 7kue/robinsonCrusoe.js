...
His hut has coordinates origin = [0, 0]. From that origin he walks a given distance d on a line that has a given angle ang with 
the x-axis. He gets to a point A. (Angles are measured with respect to the x-axis)

From that point A he walks the distance d multiplied by a constant distmult on a line that has the angle ang multiplied by a 
constant angmult and so on and on.
...
function crusoe(n, d, ang, distmult, angmult) {
   ...
   let lastY = 0;
   for (let i = 0; i < n; i++) {
      lastX += d * Math.cos(ang * Math.PI / 180);
      lastY += d * Math.sin(ang * Math.PI / 180);
      d *= distmult;
      ang *= angmult;
   }
   return [lastX, lastY];
